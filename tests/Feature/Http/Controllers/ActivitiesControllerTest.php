...
it('displays user activities page', function () {
    $activities = Activity::factory()->for($this->user)->count(5)->create();
...
it('orders activities by created_at descending', function () {
    $oldActivity = Activity::factory()->for($this->user)->create([
        'created_at' => now()->subDays(2),
        'type' => 'book.added',
    ]);

    $newActivity = Activity::factory()->for($this->user)->create([
        'created_at' => now()->subDay(),
        'type' => 'book.status.updated',
    ]);

    $response = $this->actingAs($this->user)->get(route('user.activities.index'));
...
it('preserves query string in pagination', function () {
    ...
    $response = $this->actingAs($this->user)->get(route('user.activities.index', ['test' => 'param']));
